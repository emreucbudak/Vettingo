...
{
    public static IServiceCollection AddSubscriptionCapServices(
        ...
        IConfiguration configuration)
    {
        ...
        {
            throw new InvalidOperationException(
                "RabbitMq:Port is not configured or is invalid.");
        }

        if (!int.TryParse(
                configuration[$"{CompanySubscriptionOptions.SectionName}:DefaultPlanId"],
                out int defaultPlanId) ||
            defaultPlanId <= 0)
        {
            throw new InvalidOperationException(
                "CompanySubscription:DefaultPlanId is not configured or is invalid.");
        }

        services.AddSingleton(new CompanySubscriptionOptions
        {
            DefaultPlanId = defaultPlanId
        });
        services.TryAddSingleton(TimeProvider.System);
        services.AddTransient<CompanyCreatedCapSubscriber>();
        ...
        services.AddCap(options =>
        {
            ...
            options.DefaultGroupName = "vettingo.subscription-service";
